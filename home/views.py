...
class Home(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        models = CarModel.objects.all().annotate(product_all_count=Count('model_product')).annotate(photo_count=Count('model_product', filter=Q(model_product__product_image__isnull=False), distinct=True)).annotate(video_count=Count('model_product', filter=Q(model_product__product_video__isnull=False), distinct=True)).order_by('-product_all_count')
        total_products = Product.objects.all()
        context['total_products'] = total_products.count()
        context['total_no_photo'] = total_products.filter(product_image__isnull=True).count()
